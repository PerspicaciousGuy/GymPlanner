...
export default function WorkoutSection({ date, dayName, muscleGroup, isMissed, isTomorrow, initialData, hideBadge, syncToken, onWorkoutChanged }) {
  ...
  return (
    <div className="flex flex-col gap-3 md:gap-4">
      {bothDone ? (
        <div className="bg-emerald-50/50 border border-emerald-100 rounded-xl px-3 md:px-4 py-2.5 md:py-3 flex items-center gap-3">
          <div className="w-7 h-7 md:w-8 md:h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center shadow-lg shadow-emerald-100">
        ...
        <>
          ...
          <div className="flex items-center gap-1 md:gap-2 border-b border-slate-50 mb-1 overflow-x-auto scrollbar-none">
            <button className={tabCls('am', amDone, amSkipped)} onClick={() => setActiveSession('am')} disabled={amDone || amSkipped}>
            ...
            <button className={tabCls('pm', pmDone, pmSkipped)} onClick={() => setActiveSession('pm')} disabled={pmDone || pmSkipped}>
          ...
          <div className="flex items-center gap-4 group">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 bg-slate-50/50 border border-slate-100 rounded-xl text-xs font-bold text-slate-700 placeholder:text-slate-300 focus:bg-white focus:border-indigo-200 outline-none transition-all"
              />
            </div>
            {titleSaveFlash && (
