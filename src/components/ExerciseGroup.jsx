...
export default function ExerciseGroup({ groupIndex, group, onChange }) {
  ...
  return (
    <div className="premium-card overflow-hidden transition-all duration-700 hover:shadow-2xl hover:shadow-blue-50/50 bg-white ring-1 ring-black/5">
      {/* Group Header */}
      <div className="px-8 py-5 flex items-center justify-between border-b border-gray-100/50 bg-gray-50/10">
        <div className="flex items-center gap-4">
          ...
          <h4 className="font-black text-[#1C1C1E] tracking-tight text-sm">Exercise Group {groupIndex + 1}</h4>
        </div>

        <div className="flex items-center gap-3">
          <button className="text-[10px] font-black uppercase tracking-widest text-gray-400 hover:text-blue-600 transition-colors flex items-center gap-2 px-3 py-1.5 rounded-lg hover:bg-blue-50" onClick={handleDuplicate}>
      ...
      <div className="overflow-x-auto scrollbar-thin">
        <table className="min-w-full data-table">
          <thead className="bg-[#FAFAFA]">
            <tr>
              {COLS.map((col, i) => (
                <th key={i} className="whitespace-nowrap py-4 px-6 text-left">
                  <span className="text-[10px] font-black uppercase tracking-widest text-[#AEAEC0]">
                    {col}
                  </span>
                </th>
              ))}
      ...
      {/* Footer Actions */}
      <div className="px-8 py-5 border-t border-gray-50 flex items-center justify-between bg-white text-gray-400">
        <button
          onClick={handleAddRow}
          className="text-[11px] font-black uppercase tracking-widest flex items-center gap-2 text-[#007AFF] hover:bg-blue-50 px-4 py-2 rounded-xl transition-all"
        >
